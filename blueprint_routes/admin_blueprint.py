...
@admin_blueprint.route('/admin/manageZone', methods=['GET', "POST", "PUT", "PATCH", "DELETE"])
@admin_blueprint.route('/admin/manageZone/<id>', methods=['GET', "POST", "PUT", "PATCH", "DELETE"])
@token_required
def managezone(current_user, id=None):
    ...
    if request.method == "GET":
        ...
        arra = arra + [
            {
                ...
                '$addFields': {
                    'circleName': '$circleArray.circleName'
                }
            }, {
                '$addFields': {
                    'circleName': {
        ...
        if id == None:
            ...
            if len(response['data']):
                return {
                    "status": 400,
                    "msg": "This Zone is Already Exist",
                    "icon": "error",
                }, 400
            if 'customer' in allData:
                allData['customer'] = ObjectId(allData['customer'])
            valuew = []
            for i in allData['circle'].split(","):
                valuew.append(ObjectId(i))
            allData['circle'] = valuew
            try:

                # Added="The circle Name "+allData['circleName'] +' with circle Code '+allData['circleCode']+' and band '+Band+' added in '+customere+' Customer '
                Added = "The zone "+" "+allData['zoneName'] + " Added"
                cmo.insertion("AdminLogs", {'type': 'Add', 'module': 'zone', 'actionAt': current_time(
                ), 'actionBy': ObjectId(current_user['userUniqueId']), 'action': Added})
            except Exception as e:
                print('Hello', e)

            response = cmo.insertion("zone", allData)
            return respond(response)
            ...
            try:
                ...
                Responsed = cmo.finding_aggregate("zone", arr)['data']
                allData2 = Responsed[0]
                changedThings = changedThings2(allData, allData2)

                # Added="The circle Name "+allData['circleName'] +' with circle Code '+allData['circleCode']+' and band '+Band+' added in '+customere+' Customer '
                Added = changedThings + \
                    " updated in this zone "+allData['zoneName']
                cmo.insertion("AdminLogs", {'type': 'Update', 'module': 'zone', 'actionAt': current_time(
                ), 'actionBy': ObjectId(current_user['userUniqueId']), 'action': Added})
            except Exception as e:
                print(e, 'dhhdhdhhdh')

            response = cmo.updating("zone", updateBy, allData, False)
            return respond(response)
        ...
        if id != None:
            ...
            Response = cmo.finding_aggregate("zone", arr)['data']
            Zone = None
            if len(Response):
                Zone = Response[0]['zoneName']
                Added = "Deleted This Zone "+Zone
                cmo.insertion("AdminLogs", {'type': 'Delete', 'module': 'zone', 'actionAt': current_time(
                ), 'actionBy': ObjectId(current_user['userUniqueId']), 'action': Added})

            response = cmo.deleting("zone", id, current_user['userUniqueId'])
            return respond(response)
        else:
            return jsonify({"msg": 'Please Provide Valid Unique ID'})
...
@admin_blueprint.route('/admin/manageProjectType/<id>', methods=['GET', "POST", "PUT", "PATCH", "DELETE"])
@admin_blueprint.route('/admin/manageProjectType/<id>/<rowId>', methods=['GET', "POST", "PUT", "PATCH", "DELETE"])
@token_required
def manageprojecttype(current_user, id=None, rowId=None):
    ...
    if request.method == "GET":
        ...
        if id != None:
            ...
            if ("t_sengg" in allData):
                ...
                if (siteId and prAll_rfi and auto_created and drop_down):
                    try:
                        ...
                        Responsed = cmo.finding_aggregate(
                            "projectType", arr)['data']

                        allData2 = Responsed[0]
                        changedThings = changedThings3(allData2, allData)

                        tt = []
                        ttt3 = {'dataType': 'Input Type',
                                'required': 'Mandatory', 'Status': 'Status'}
                        for i in changedThings:
                            if i['changedKey'] == 'fieldName':
                                tt.append("Field Name  changed from " +
                                          i['Oldvalue']+' to ' + i['Newvalue'])
                            else:
                                print('iiiii', i)
                                tt.append('for Field Name '+i['changedKey']+' type changed ' +
                                          ttt3[i['field']] + ' from '+i['Oldvalue']+' to '+i['Newvalue'])
                        # Added="The circle Name "+allData['circleName'] +' with circle Code '+allData['circleCode']+' and band '+Band+' added in '+customere+' Customer '

                        Added = "For Project  Type " + allData2['projectType']+' and Sub Project ' + \
                            allData2['subProject'] + \
                                'in Site Engg. These things are updated :-' + \
                            str(tt)
                        cmo.insertion("AdminLogs", {'type': 'Update', 'module': 'Manage  Project Type', 'actionAt': current_time(
                        ), 'actionBy': ObjectId(current_user['userUniqueId']), 'action': Added})
                    except Exception as e:
                        print(e, 'dhhdhdhhdh')
                ...
                if (auto_created and drop_down):
                    try:
                        ...
                        Added = "For Project  Type " + allData2['projectType']+' and Sub Project ' + \
                            allData2['subProject'] + \
                                'in Tracking These things are updated :-' + \
                            str(tt)
                        cmo.insertion("AdminLogs", {'type': 'Update', 'module': 'Manage  Project Type', 'actionAt': current_time(
                        ), 'actionBy': ObjectId(current_user['userUniqueId']), 'action': Added})
                    except Exception as e:
                ...
                if (auto_created and drop_down):
                    try:
                        ...
                        Added = "For Project  Type " + \
                            allData2['projectType']+' and Sub Project ' + \
                                allData2['subProject'] + \
                            ' Issues These things are updated :-'+str(tt)
                        cmo.insertion("AdminLogs", {'type': 'Update', 'module': 'Manage  Project Type', 'actionAt': current_time(
                        ), 'actionBy': ObjectId(current_user['userUniqueId']), 'action': Added})
                    except Exception as e:
                        print(e, 'dhhdhdhhdh')
            ...
            elif ("t_sFinancials" in allData):

                for datew in allData["t_sFinancials"]:
                    for key, value in datew.items():
                        if key in ["fieldName", "dropdownValue"]:
                            datew[key] = value.strip()
                ...
                try:
                    ...
                    Added = "For Project  Type " + allData2['projectType']+' and Sub Project ' + \
                        allData2['subProject'] + \
                            'in Financial These things are updated :-'+str(tt)
                    cmo.insertion("AdminLogs", {'type': 'Update', 'module': 'Manage  Project Type', 'actionAt': current_time(
                    ), 'actionBy': ObjectId(current_user['userUniqueId']), 'action': Added})
                except Exception as e:
                    print(e, 'dhhdhdhhdh')
                ...
                try:
                    ...
                    allData2 = Responsed[0]
                    changedThings = changedThings4(allData2, allData)

                    tt = []
                    ttt3 = {'dataType': 'Input Type', 'required': 'Mandatory', 'Status': 'Status', 'WCC Sign off': 'WCC Sign off',
                            'Completion Criteria': 'Completion Criteria', 'Predecessor': 'Predecessor', 'Estimated Time (Days)': 'Estimated Time (Days)'}
                    for i in changedThings:
                        if i['changedKey'] == 'fieldName':
                            tt.append("Field Name  changed from " +
                                      i['Oldvalue']+' to ' + i['Newvalue'])
                            ...
                            tt.append('for Field Name '+i['changedKey']+'  changed ' + ttt3[i['field']] + ' from '+str(
                                i['Oldvalue'])+' to '+str(i['Newvalue']))
                    # Added="The circle Name "+allData['circleName'] +' with circle Code '+allData['circleCode']+' and band '+Band+' added in '+customere+' Customer '

                    Added = "For Project  Type " + allData2['projectType']+' and Sub Project ' + \
                        allData2['subProject'] + \
                            'in Milestone These things are updated :-'+str(tt)
                    cmo.insertion("AdminLogs", {'type': 'Update', 'module': 'Manage  Project Type', 'actionAt': current_time(
                    ), 'actionBy': ObjectId(current_user['userUniqueId']), 'action': Added})
                except Exception as e:
                    print(e, 'dhhdhdhhdh')
...
def project(current_user, id=None, rowId=None):
    ...
    if request.method == "GET":
        ...
        if rowId == None:
            ...
            allData['custId'] = id
            allData['projectGroup'] = ObjectId(allData['projectGroup'])
            response = cmo.insertion("project", allData)
            evl.newproject(
                response['operation_id'], allData['projectId'], current_user['userUniqueId'])
            return respond(response)
...
@admin_blueprint.route('/vishal', methods=['GET', "POST", "PUT", "PATCH", "DELETE"])
@token_required
